...
RSpec.describe Regent::Agent, :vcr do
  ...
  context "without a tool" do
    ...
    it "stores session history" do
      ...
      expect(agent.session.spans.count).to eq(2)
      expect(agent.session.spans.first.type).to eq(Regent::Span::Type::LLM_CALL)
      expect(agent.session.spans.first.output).to eq("Thought: I need to find out what the capital of Japan is. \nAction: I will recall my knowledge about countries and their capitals. \nObservation: The capital of Japan is Tokyo. \n\nThought: I have the answer now.\nAnswer: The capital of Japan is Tokyo.")
  ...
  context "with a tool" do
    ...
    let(:agent) { Regent::Agent.new(llm, [PriceTool.new('price_tool', 'Get the price of Crypto currencies')]) }
    ...
    it "stores messages within a session" do
      ...
      expect(agent.session.messages).to eq([
        { role: :system, content: Regent::Arch::React::SYSTEM_PROMPT.gsub("%{tools}", "price_tool") },
    ...
    it "stores session history" do
      ...
      expect(agent.session.spans.count).to eq(4)
      expect(agent.session.spans.first.type).to eq(Regent::Span::Type::LLM_CALL)
      expect(agent.session.spans[1].type).to eq(Regent::Span::Type::TOOL_EXECUTION)
      expect(agent.session.spans[2].type).to eq(Regent::Span::Type::LLM_CALL)
      expect(agent.session.spans.last.type).to eq(Regent::Span::Type::ANSWER)
    end
  end
end
